                             ...
                             col = NULL, startpos = 0, ...)
{
  ...
  if (length(i <- which(dx == 1))) {
    theta <- seq(0, 2*pi, length.out = 100) # Generate 100 points for a smooth circle
    xc <- cos(theta) * radius + xpos
    yc <- sin(theta) * radius + ypos
    polygon(xc, yc, border = par("fg"), col = col[i], lwd = 0.5) # Set lwd here
  } else {
    ...
    for (i in seq_len(nx)) {
      ...
      polygon(xc, yc, col = col[i],lwd=0.5, ...)
    }
  }
}
