...
```{r}
fishdata_name<-file.path(root,'input_data/04_Phylogeny_And_Admixture/mormyrid_specimens_with_phenotypes.csv')
...
plot_name<-file.path(root,'output_data/04_Phylogeny_And_Admixture/mormyrid_character_map.svg')
svg(plot_name, width=12)  # Adjust width as needed
arctree(mormyrotree_pruned,0.4)
tiplabels(pie=to.matrix(new_phenos[mormyrotree_pruned$tip.label], levels(new_phenos)), piecol=cols, cex=0.2)
nodelabels(node=1:mormyrotree_pruned$Nnode+Ntip(mormyrotree_pruned),
...
```{r,fig.height=12}
plot_name<-file.path(root,'output_data/04_Phylogeny_And_Admixture/mormyrid_parsimony.svg')
svg(plot_name, width=12)  # Adjust width as needed
