...
# Skip the header and read the file line by line using a file descriptor
{
  ...
  while IFS=$'\t' read -r specno population r1 r2 phenotype; do
    ...
    echo sbatch --output=${slurmoutdir}/align_${specno}.out --export=READ1="${readsdir}/${specno}/tr_${specno}_1P.fastq.gz",READ2="${readsdir}/${specno}/tr_${specno}_2P.fastq.gz",OUTDIR=${outdir}/${specno},SAMPLE=${specno} ${root}/code/09_RNASeq/run_star_rnaseq.sb;
    sbatch --output=${slurmoutdir}/align_${specno}.out --export=READ1="${readsdir}/${specno}/tr_${specno}_1P.fastq.gz",READ2="${readsdir}/${specno}/tr_${specno}_2P.fastq.gz",OUTDIR=${outdir}/${specno},SAMPLE=${specno} ${root}/code/09_RNASeq/run_star_rnaseq.sb;
 fi
  done
} < "$input_file"
