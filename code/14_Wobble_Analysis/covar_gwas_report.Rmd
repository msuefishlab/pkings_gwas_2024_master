...
pk.lmm.bed.f <- pk.lmm.bed %>% 
  ...
  filter(num.snps.above.threshold > 2) %>% 
  ungroup() %>% 
...
```{r}
ggsave(file.path(root,"output_data/14_Wobble_Analysis",params$data_path,paste0(params$data_path,".gwas.svg")), plot = man_final, width = 12, height = 4, units = "in")
```