...
```{r}
admix.file=file.path(root,"output_data/13_Structure/",params$outname,"admixture_long_results.header.txt")
cv.file=file.path(root,"output_data/13_Structure/",params$outname,"cv_errors.txt")
...
p1<-ggplot(subset(admix_data,MAXK==3), aes(factor(Sample), Q, fill = factor(GROUP))) +
...
p2 <- ggplot(subset(admix_data, MAXK == 3), aes(x = factor(Sample), y = 0.5, fill = factor(Phenotype))) +
...
```
```{r}
ggsave(file.path(root,'output_data/13_Structure/',paste0(params$outname,'_final_admixture.pdf')), plot = combined_plot, width = 12, height = 4, units = "in")
```