...
get_gene_info_in_batches <- function(gene_ids, batch_size = 50, delay = 1) {
  gene_info_list <- list()
  
  # Split gene IDs into batches
  gene_batches <- split(gene_ids, ceiling(seq_along(gene_ids) / batch_size))
  
  # Loop through each batch
  for (i in seq_along(gene_batches)) {
    cat("Processing batch", i, "of", length(gene_batches), "\n")
    
    # Get the gene summaries for the current batch
    gene_summaries <- entrez_summary(db = "gene", id = gene_batches[[i]])
    
    # Extract relevant information for each gene in the batch
    batch_info <- lapply(gene_summaries, function(x) {
      c(GeneID = x$uid, Symbol = x$name, Description = x$title, description = x$description)
    })
    
    # Combine the current batch's info with the rest
    gene_info_list <- c(gene_info_list, batch_info)
...
# Fetch gene information in batches (adjust batch_size and delay as needed)
gene_info <- get_gene_info_in_batches(overlapping_annotations_df$gid, batch_size = 50, delay = 1)
