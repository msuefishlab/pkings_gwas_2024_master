---
title: "SweeD Genome-Wide Sweep Analysis"
...
This notebook analyzes genome-wide selective sweep patterns using **SweeD** Composite Likelihood Ratio (CLR) statistics. We compare CLR values in GWAS peaks versus genome-wide background to determine how "sweepy" our GWAS loci are relative to the rest of the genome.

**SweeD advantages over SweepFinder2:**

- 21x faster sequential execution
- Multi-threaded parallel computation
- Native VCF support (no conversion step)
...
for (pop in pops) {
  file <- file.path(root, "output_data/12_Sweep_Detection/sweed/merged",
...
peak_clr <- read_tsv(
  file.path(root, "output_data/12_Sweep_Detection/sweed/peak_clr_values.txt"),
...
peak_summary <- read_tsv(
  file.path(root, "output_data/12_Sweep_Detection/sweed/peak_clr_summary.txt"),
  show_col_types = FALSE
)
```
...
ggsave(
  file.path(root, "output_data/12_Sweep_Detection/sweed/clr_manhattan.pdf"),
  p_manhattan,
  width = 14, height = 10, units = "in"
)
ggsave(
  file.path(root, "output_data/12_Sweep_Detection/sweed/clr_manhattan.png"),
  p_manhattan,
  width = 14, height = 10, units = "in", dpi = 300
)

cat("\nFigures saved:\n")
cat("  PDF: output_data/12_Sweep_Detection/sweed/clr_manhattan.pdf\n")
cat("  PNG: output_data/12_Sweep_Detection/sweed/clr_manhattan.png\n")
```
...
# Save high-resolution version
ggsave(
  file.path(root, "output_data/12_Sweep_Detection/sweed/clr_zoomed_peaks.pdf"),
  p_zoomed,
  width = 14, height = 10, units = "in"
)
ggsave(
  file.path(root, "output_data/12_Sweep_Detection/sweed/clr_zoomed_peaks.png"),
  p_zoomed,
  width = 14, height = 10, units = "in", dpi = 300
)

cat("\nZoomed peak figures saved:\n")
cat("  PDF: output_data/12_Sweep_Detection/sweed/clr_zoomed_peaks.pdf\n")
cat("  PNG: output_data/12_Sweep_Detection/sweed/clr_zoomed_peaks.png\n")
```
