...
params:
  data_path: APA_BEN_BAM_TP2_BP0_WOB1
  output_file: NULL
  MAF_thresh: 0.05
---
...
groups_path<-file.path(root,"input_data","08_Peak_Analysis","phenotype_assignments_APA_BAM_BEN_WOB_INCL.txt")
...
ld_peaks<-ld_data %>%
  ...
  filter(R2 > 0.9) %>%
  select(Peak = peak, CHROM=CHR_B, POS=BP_B, snp_id=SNP_B, R2)
...
index_snps <- df.bed %>%
  ...
  select(Peak = peak, CHROM, POS, snp_id)
...
results <- data.frame()
  ...
  for (p in peaks) {
    ...
    filter(peak == p) %>%
      ...
      select(PN, snp_id, genotype, Phenotype) %>%
      drop_na()  # remove missing genotypes

    # Add numeric phenotype column here
    subset <- subset %>%
      mutate(num_pheno = ifelse(Phenotype == "Wobble" | Phenotype =="TP", 1, 0))

    # Pivot wider to get genotypes in columns
    geno_wide <- subset %>%
      select(-Phenotype) %>%  # drop original group label
      pivot_wider(names_from = snp_id, values_from = genotype) %>%
      distinct()

    # Make sure phenotype exists and has two values
    if (!"num_pheno" %in% colnames(geno_wide)) next
    if (length(unique(geno_wide$num_pheno)) < 2) next

    # Build formula
    snp_cols <- setdiff(colnames(geno_wide), c("PN", "num_pheno"))
    if (length(snp_cols) < 1) next

    formula <- as.formula(paste("num_pheno ~", paste(snp_cols, collapse = " + ")))
    ...
    # Store result
    results <- bind_rows(results, tibble(Peak = p, R2 = r2))
  }


```
```{r}
ggplot(results, aes(x = Peak, y = R2)) +
  geom_bar(stat = "identity", position = "dodge") +
  theme_minimal() +
  labs(y = "McFadden's pseudo-R²", title = "Variance Explained per Peak per Group")
```
```{r}
results <- results[order(-results$R2, decreasing = FALSE), ]
...
check_plot_data <- annotated_geno %>%
  filter(peak == peak_to_check)

ggplot(check_plot_data, aes(x = Phenotype, y = genotype, color=Phenotype)) +
...
```{r}
plot_name<-file.path(root,'output_data/08_Peak_Analysis/mcfaddenrsquared.svg')
svg(plot_name, width=6)  # Adjust width as needed
