...
index_snps_simp$maximum <- index_snps_simp$BP_A + 25000
index_snps_simp$minimum <- index_snps_simp$BP_A - 25000
...
# The output data frame now includes the peak information for faceting
  ...
  geno_plot<-create_phylo_geno_plot_multi(tree,df.genos.sub,peaks_per_plot = 3)
...
# plot fst
    ...
    bxf<-ggplot(fst_res_group, aes(x = comparison, y = avg_wc_fst, fill = compare_color,alpha=type)) +
      ...
      facet_wrap(~ peak, scales = "free_x",ncol=3) +
...
for (group in peak_groups) {
  ...
  scale_color_gradientn(
    ...
    geom_gene_arrow(data = overlapping_annotations_df_group, 
                    mapping = aes(xmin = start / 1e6, xmax = end / 1e6, y = maxp+2,forward = forward),
                    fill = "darkgreen",
                    alpha=0.4,
                    show.legend = FALSE) +
  ...
  geom_text_repel(
    ...
    aes(x = (start / 1e6 + end / 1e6) / 2, y = maxp + 2, label = label_name),
  ...
  geom_text_repel(
    ...
    theme(
      ...
      axis.text.x = element_text(size = rel(1.2), family = "Arial", angle = 90, hjust = 1),
...
for (i in seq_along(snp_plot_list)) {
  thebigplot_list[[i]] <- wrap_plots(
    (wrap_plots(snp_plot_list[[i]], geno_plot[[i]], ncol = 1, heights = c(0.75, 2))), # Left stack
...
for (i in seq_along(snp_plot_list)) {
  ggsave(file.path(root,"output_data/08_Peak_Analysis",params$data_path,paste0(params$data_path,".peak_analysis_",i,".svg")),plot=thebigplot_list[[i]],limitsize = FALSE,width = 36, height=24)
}
