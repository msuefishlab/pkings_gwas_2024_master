...
params:
  data_path: APA_BAM_BEN_TPBP0_WOB1
  output_file: NULL
...
```{r}
geno_path<-file.path(root,"output_data","08_Peak_Analysis",params$data_path,paste0(params$data_path,".plink.allelic.bin.pos.genos"))
peaks_path<-file.path(root,"output_data","06_Association",params$data_path,paste0(params$data_path,"_PLINK_ALLELIC_PEAKS.bed"))
freq_file<-file.path(root,"output_data","06_Association",params$data_path,paste0(params$data_path,"_PLINK_assoc.frq"))
logp_path<-file.path(root,"output_data","06_Association",params$data_path,paste0(params$data_path,"_PLINK_ALLELIC_SNPS_IN_PEAKS_TOP250.txt"))
...
ldpath<-file.path(root,"output_data","06_Association",params$data_path,paste0(params$data_path,"_plink_allelic_index_snps.ld.ld"))
...
significant<-thr_by_test_ord[thr_by_test_ord$TEST=="ALLELIC",]$significant
suggestive<-thr_by_test_ord[thr_by_test_ord$TEST=="ALLELIC",]$suggestive
...
pk.lmm.filt <- pk.lmm %>% filter(TEST =="ALLELIC") %>% filter(log_p > 1) %>% filter(MAF > params$MAF_thresh) %>% filter((!grepl("ups", CHR)) & !is.na(log_p)) %>% arrange(numeric_chr)
```
...
gwas_ld_data<-left_join(pk.lmm.filt,ld_data,by=c("SNP"="SNP_B"))

# Get the Data inside the Peaks
pk.lmm.peaks.out<-read.table(file.path(peak_data_path,paste0(params$data_path,"_PLINK_ALLELIC_SNPS_IN_PEAKS.txt")),header=T)
...
# Simplify the output
index_snps_simp <- index_snps[, c(17, 2, 1, 10, 8)]
colnames(index_snps_simp) <- c("PEAK", "SNP_A", "CHR_A", "BP_A", "p")
...
write.table(overlapping_annotations_for_output,file.path(peak_data_path,paste0(params$data_path,"_GENES_IN_PEAKS_PLINK_ALLELIC.txt")), sep = "\t", quote = F, row.names = F, col.names = T)
```
...
plots_by_peak <- lapply(peaks, function(pk) {
  ...
  ggplot(
    ...
    geom_tile() +
    # outline the max pos with a blue box
    geom_tile(
      data = df_pk_outline,
...
maxp<-max(-log10(pk.lmm.filt_new$p))
...
# Extract unique peaks and split into groups
  ordered_levels <- index_snps_simp$PEAK[order(index_snps_simp$p)]
...
for (group in peak_groups) {
  ...
  snp_plot <- ggplot() +
    geom_point(data = peak.pk.lmm_group, mapping = aes(x = ps / 1e6, y = -log10(P)), color = "grey") +
    geom_point(data = peak.gwas_ld_data_group, mapping = aes(x = BP_B / 1e6, y = -log10(P), color = R2)) +
