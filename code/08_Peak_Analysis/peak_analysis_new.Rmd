...
```{r}
tree_path <- file.path(root, "output_data", "04_Phylogenomics", "pkings_iqtree_genes_astral_gcf_scf_tree.cf.tree.ultra.time.nex")
...
```{r,fig.height=12}
keep_pops <- c("APA", "BAM", "BEN", "BAVA","BIK","MOV","DOV","DOG")
...
plots_by_peak <- lapply(peaks, function(pk) {
  ...
  keep_names <- c(
  "BEN_699", "BEN_705", "BEN_706", "BEN_707", "BEN_708",
  "BIK_6898", "BIK_6900", "BIK_6901",
  "DOG_649",
  "DOV_664", "DOV_665", "DOV_666", "DOV_667",
  "MOV_6716", "MOV_6718"
)
...
overlapping_annotations_df_pretty <- overlapping_annotations_df %>% left_join(pretty_names,by="Name")

maxp <- max(-log10(pk.lmm.filt_new$p_lrt))

# Replace NA values in `external_gene_name` with `gene_id`
overlapping_annotations_df_pretty <- overlapping_annotations_df_pretty %>%
  mutate(label_direction = ifelse(row_number() %% 2 == 0, "up", "down")) # Alternate "up" and "down"

peaks_per_plot <- 6

# Extract unique peaks and split into groups
index_snps_simp$PEAK<-as.numeric(index_snps_simp$PEAK)
ordered_levels <- index_snps_simp$PEAK[order(index_snps_simp$PEAK)]
...
for (group in peak_groups) {
  ...
  overlapping_annotations_df_group <- overlapping_annotations_df_pretty %>% filter(paste0("peak", PEAK) %in% group)
  ...
  # Derive the numeric order for this group, e.g. c(1,2,3,4,5,6)
group_ids <- as.integer(sub("^peak", "", group))

# Relevel PEAK in every data frame used for faceting
peak.pk.lmm_group <- peak.pk.lmm_group %>%
  mutate(PEAK = factor(PEAK, levels = group_ids, ordered = TRUE))

peak.gwas_ld_data_group <- peak.gwas_ld_data_group %>%
  mutate(PEAK = factor(PEAK, levels = group_ids, ordered = TRUE))
...
overlapping_annotations_df_group <- overlapping_annotations_df_group %>%
  ...
  snp_plot <- ggplot() +
    ...
    geom_text_repel(
      ...
      aes(x = (start / 1e6 + end / 1e6) / 2, y = ypos, label = display_name),
    ...
    geom_text_repel(
      ...
      aes(x = (start / 1e6 + end / 1e6) / 2, y = ypos - 1, label = display_name),
...
for (i in seq_along(snp_plot_list)) {
  ggsave(file.path(root, "output_data/08_Peak_Analysis", params$data_path, paste0(params$data_path, ".peak_analysis_snps_only", i, ".svg")), plot = snp_plot_list[[i]], limitsize = FALSE, width = 14, height = 8)
}
